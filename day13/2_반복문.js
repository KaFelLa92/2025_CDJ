...
// 배열 내 모든 점수 조회 : 0부터 마지막인덱스까지 조회
// for (let index = 0 ; index <= 마지막인덱스 ; index++ ){}
/*

let scores = [85, 92, 78, 65, 95];
let sum = 0; // 누적 총합계를 구해야하므로 for 밖에 선언
for( let index = 0 ; index <= scores.length - 1 ; index++ ){
    // index는 0부터 마지막인덱스 이하까지 1씩 증가 반복
    // let sum = 0; 여기에 합계 변수를 선언하면 안 된다. : 합계변수는 누적해야하므로 선언 반복 X
    sum += scores[index]; // 배열 내 index번째의 요소 값을 더한다.
    // 평균 계산은 반복할 필요가 없다.
} // for end
...
for (let index = 0; index <= scores.length - 1; index++) {
    ...
    console.log(`${index + 1}번 학생은 ${value}점입니다.`)
    if (value == 100) {    // 만약 해당 값이 100점이면 
        console.log(`${value}점 만점자를 찾았습니다! 만점자는 ${index + 1}번 학생입니다.`)
        break; // 가장 가까운 for{} 탈출
    }   // if end
}   // for end
...
for (let index = 0; index <= scores.length - 1; index++) {
    ...
    if (num < 60) {
        continue;   // if문 만족하면 증감식으로 이동
    }
    console.log(`${index + 1}번 학생은 ${num} 점 받았습니다.`)
}
*/
...
for (let index = 0; index <= bloodTypes.length - 1; index++) {
    let blood = bloodTypes[index];  // index번째 혈액형 호출
    if (blood == 'A' ) { // 혈액형 A이면
        countA++; // vs countA += 1 vs countA = countA + 1
    }
}
console.log(`혈액형 A타입은 ${countA}명입니다.`);
*/
...
for (let index = 0; index <= nameArray.length -1 ; index++) { // 점수 정보
...
/*
    for (let circle = 1; circle <= 10; circle++) { // 찬 동그라미 정보
        figure += "●"
    }
    for (let emptyC = 1; emptyC <= 10; emptyC--){   // 빈 동그라미 정보
