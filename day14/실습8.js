...
/* HTML 출력 예시:
   ...
   전지적 독자 시점 ★★★★★★★★★☆
    ...
    for문 바깥에 output 변수 추가
    for문 돌려서  웹툰스[i]를 웹툰으로 변수 설정
    웹툰.레이팅 값을 parseInt를 사용해 정수값만 출력.
*/
...
for (let i = 0; i <= webtoons.length - 1; i++) {
    let webtoonRate = webtoons[i].rating    // 웹툰은 웹툰스의 인덱스값
    html += `<div> ${webtoons[i].title} `;
    html += '\n';

    for (let r = 0; r <= parseInt(webtoonRate) - 1; r++) {
        html += '★'; // output에 찬 별 대입
    }
    for (let j = 0; j <= 10 - parseInt(webtoonRate) - 1; j++) {
        html += `☆`; // output에 빈 별 대입
    }
...
let html = '';
html += `<h1> 202408 부평구 맛집 현황 테이블 </h1> <table border="1"> <thead> <tr> <th> 소재지 </th> <th> 업 소 명 </th> <th> 업태 </th> <th> 연번 </th> <th> 전화번호 </th> <th> 메뉴 </th> </tr> </thead> <tbody>`

for (let i = 0; i <= response.data.length - 1; i++) {
    let respon = response.data[i]
    html += `<tr> <td> ${respon["소재지"]} </td> <td> ${respon["업 소 명"]} </td> <td> ${respon["업태"]} </td> <td> ${respon.연번} </td> <td> ${respon["전화번호"]} </td> <td> ${respon["지정메뉴"]} </td>`
}
html += `</tbody> </table>`
document.write(html);
