...
/*
const person = { name : "최동진" , age : 34 , city : "부천" };
console.log(person);    // 문자열과 같이 쓰면 object로 나옴
*/
...
// 1. 객체 내 모든 값 반환 : Object.valuse( 변수명 )
...
person.email = "kimcording@gmail.com"; // 변수명.속성명 = 새로운자료; -> 추가하기
delete person.age;  // delete 변수명.속성명 -> 삭제하기
...
console.log(product.price * product.stock); // 변수명.속성명을 통해 특정 속성 자료를 호출할 수 있다.
*/
...
console.log(user[`job-title`]); // 변수명['속성명'] -> 문자열일 때 속성명 값 호출
*/
...
console.log(student.address.city);  //  변수명.속성명.속성명 -> 값 (폴더 이동하듯 위치 이동 가능)
*/
...
console.log("level" in character);  // "속성값" in 변수로 true, false 판독 가능
*/
...
console.log(Object.keys(country));      // Object.keys(변수명) -> 변수명을 배열로 출력
console.log(Object.values(country));    // Object.values(변수명) -> 값을 배열로 출력
...
const names = Object.keys(inventory); // 모든 상품의 이름
for ( let i = 0; i <= names.length - 1; i++){ // [ '애플', '바나나' ...
    let name = names[i] // 애플 바나나 오렌지 그레이프 순차적 출력
    stock = inventory[name]; // 재고는 애플[15] , 바나나[5] .... 
    if( stock < 10){
        console.log(name)
    }
}
...
products = Object.keys(inventory)
for (i = 0; i <= products.length - 1; i++) {
    let product = products[i];  // 변수 생성 애플 바나나 오렌지 그레이프 출력
    stock = inventory[product]  // 열 계산 : 재고는  위와 합치면 애플 15 바나나 5 오렌지 20 그레이프 8 순차적 출력
...
for(i = 0; i <= students.length -1; i++){
    const student = students[i] // 객체 배열 인덱스 값을 student로 지정
    console.log(student.name + ":" + student.major);    // 김철수 : 컴퓨터공학
}
*/
...
for (let i = 0; i <= data.length - 1; i++) {
    ...
    const obj = data[i];    // i번째 객체(동별 인구정보) 호출
    html += `<tr>
                <td> ${ obj.동별 } </td> <td> ${ obj.세대수 } </td> <td> ${ obj["인구수(계)"] } </td>
