...
/* 
    [ 작업순서 ]
        ...
        3. (백) 함수(기능)설계
            (1) '등록' 함수 만들기 : 등록버튼 클릭했을 때 입력받은 3가지 정보를 배열에 저장
                함수명 :        (아무거나) '등록함수'
                매개변수 :      x document.querySelector로 처리
                반환값 :        x
                로직 :          1. input에서 입력받은 값 가져오기
                                2. 입력값 3개를 객체로 구성
            ...
            (2) '전체조회' 함수 
                ...
                실행조건 :      1. 페이지 열렸을 때(최초1번) 2. 등록 성공했을 때(새로고침)
...
let 코드자동번호 = 가계부목록 [ 마지막인덱스 ].코드 // 2 // 현재 초기배열의 마지막 객체 번호



// [3] 기능(함수단위) 구현
// 1. 등록함수 정의 : 실행 : 등록버튼 onclick 했을 때.
function 등록함수() {
    console.log(`--- 등록함수 exe ---`) // 2. 함수 onclick 확인. 상수, 변수 추가할 때마다 확인해라
    // 3. input에서 입력받은 값 '각각' 가져오기
    ...
    // 5. 원하는 속성 구성으로 객체 만들기 , 설계: { 코드 : , 날짜 : , 항목명 : , 금액 : }
        // * 코드자동번호에 1 증가한 후 대입한다.
        코드자동번호++; // 번호 삭제하면 하나씩 빠지게됨
...
// 2. 전체조회함수 , 실행조건 : 1. JS 열렸을 때(최초 1번) 2. 등록 성공했을 때.
...
function 전체조회함수() {
    ...
    let html = ``;
    for (let i = 0; i <= 가계부목록.length - 1; i++) {   // (1) 배열내 모든(for) 객체의 정보 호출
        const obj = 가계부목록[i];  // i번째 객체(정보) 호출
        html += `<tr> 
                    <td> ${obj.날짜} </td> 
                    <td> ${obj.항목명} </td> 
                    <td> ${obj.금액} </td> 
    ...
    // 5. 출력      , innerHTML
    contentBody.innerHTML = html;   // 반복문 이용하여 객체를 HTML 형식으로 표현한 누적 HTML을 대입한다.
}   // func end
