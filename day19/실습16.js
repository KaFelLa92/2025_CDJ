...
function signup() {
    ...
    // 3. 객체화
    let no = 1; // 회원번호 초기값
    // sessionStorage에서 userList 가져오기
    // (1) sessionStorage userList 가져오기
    let userList = sessionStorage.getItem('userList'); // .getItem('속성명/key')
    // (2) 존재하지 않으면 (배열) 새로 생성 , 존재하면 타입변환
    if (userList == null) { // 해당 속성명(userList)이 존재하지 않으면 
        userList = [];  // 새로운 배열 생성
        // no(회원번호) 1 사용한다.
    } else {    // 존재하면 JSON(배열타입)으로 변환하기.
        userList = JSON.parse(userList);    // 자바스크립트 객체를 숫자로 바꿔줌
        no = userList[userList.length - 1].no + 1; // 배열내 마지막 인덱스의 회원번호 + 1
    }
    const obj = { no: no, id: id, pw: pw }; console.log(obj);
    // 4. 배열 저장
    userList.push(obj); console.log(userList);
    alert(`회원 등록 성공`); // 알림
    // sessionStorage에서 userList 저장하기
    // (1) 배열타입을 JSON 문자열 타입으로 변환
    let jsonData = JSON.stringify(userList);   // 자바스크립트 객체를 문자열로 바꿔줌
    // (2) sessionStorage에 userList 속성명으로 배열 저장하기
    sessionStorage.setItem('userList', jsonData);    // userList 변수값
...
function login() {
    ...
    // 2. 입력마크업 객체내 입력값 가져오기
    const id = loginId.value; console.log(id);
    const pw = loginPw.value; console.log(pw);
    // 3. 기존배열(회원목록)내 입력받은 값과 일치하는 정보 찾기(비교) , <for>문 사용
    // sessionStorage에서 userList 가져오기
    let userList = sessionStorage.getItem('userList');  // 'userList'라는 속성값 가져오기
    if (userList == null) { // 만약에 'userList' 이름의 속성이 존재하지 않으면
        userList = [];          // 새로운 배열 생성
    } else {    // userList 속성명이 존재하면 JSON(배열) 타입으로 변환하기.
        userList = JSON.parse(userList);
    }
    for (let i = 0; i <= userList.length - 1; i++) {
        const user = userList[i]; // i번째 회원정보 (객체)
        if (user.id == id && user.pw == pw) {
            // 만약 i번째 회원아이디가 입력한 아이디와 같고 회원비밀번호가 입력한 비밀번호와 같으면
            alert('로그인 성공');
            return; // 강제 함수(반복문) 종료
        }   // 못 찾는 경우를 else로 만들면 안 됨.
    }
    // 4. 못 찾았다.
    alert('로그인 실패')
